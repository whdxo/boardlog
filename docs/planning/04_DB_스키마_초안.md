...
├── profile_image_url
├── bio
├── social_provider (kakao | apple | google | null)
├── social_id
├── created_at
└── updated_at
```
...
## Purchase Links (구매 링크)

```
purchase_links
├── id (PK)
├── board_game_id (FK)
├── shop_name (string - 네이버 쇼핑, 쿠팡, 공식몰 등)
├── url (string)
├── price (int, nullable - 원 단위)
...
├── played_at (date)
├── player_count (int)
├── players (json array, nullable - 함께한 사람 이름 목록)
├── location (string, nullable - 플레이 장소)
...
## Funding Backers (후원 내역)

```
funding_backers
├── id (PK)
├── user_id (FK → users)
├── project_id (FK → funding_projects)
├── reward_id (FK → funding_rewards, nullable)
├── amount (bigint)
├── status (pending | paid | refunded | cancelled)
├── created_at
└── updated_at

[unique: user_id + project_id + reward_id]
```
